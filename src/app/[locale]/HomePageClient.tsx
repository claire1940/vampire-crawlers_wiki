    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12 scroll-reveal">
            {t.modules.lucidBlocksFarmingAndGrowth.eyebrow && (
              <p className="text-sm uppercase tracking-[0.18em] text-[hsl(var(--nav-theme-light))] mb-3">
                {t.modules.lucidBlocksFarmingAndGrowth.eyebrow}
              </p>
            )}
            <h2 className="text-4xl md:text-5xl font-bold mb-4">
          ...
          {Array.isArray(t.modules.lucidBlocksFarmingAndGrowth.items) && t.modules.lucidBlocksFarmingAndGrowth.items.length > 0 ? (
            <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {t.modules.lucidBlocksFarmingAndGrowth.items.map((item: any, index: number) => {
                const cardIcons = [Sparkles, TrendingUp, Star, Hammer, Package, Gamepad2, Settings, Home, BookOpen]
                const Icon = cardIcons[index % cardIcons.length]
                const tierClass =
                  item.tier === 'S'
                    ? 'bg-[hsl(var(--nav-theme-light)/0.18)] border-[hsl(var(--nav-theme-light)/0.45)] text-[hsl(var(--nav-theme-light))]'
                    : item.tier === 'A'
                      ? 'bg-[hsl(var(--nav-theme)/0.18)] border-[hsl(var(--nav-theme)/0.45)] text-[hsl(var(--nav-theme-light))]'
                      : 'bg-[hsl(var(--nav-theme)/0.1)] border-[hsl(var(--nav-theme)/0.3)] text-muted-foreground'
                ...
                return (
                  <article key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${tierClass}`}>
                        {item.tier || 'Tier'}
                      </span>
                    ...
                    {item.recipe && (
                      <p className="text-sm text-muted-foreground mb-2">
                        <span className="text-[hsl(var(--nav-theme-light))]">Recipe:</span> {item.recipe}
                      </p>
                    )}
                    {item.best_for && (
                      <p className="text-sm text-muted-foreground mb-3">
                        <span className="text-[hsl(var(--nav-theme-light))]">Best For:</span> {item.best_for}
                      </p>
                    )}
                    {item.why_it_ranks_here && (
                      <p className="text-sm text-muted-foreground mb-3">{item.why_it_ranks_here}</p>
                    )}
                    {item.standout_detail && (
                      <div className="p-3 rounded-lg border border-[hsl(var(--nav-theme)/0.3)] bg-[hsl(var(--nav-theme)/0.06)]">
                        <p className="text-xs uppercase tracking-wide text-[hsl(var(--nav-theme-light))] mb-1">
                          Standout Detail
                        </p>
                        <p className="text-sm text-muted-foreground">{item.standout_detail}</p>
                      </div>
                    )}
                  </article>
                )
          ...
          {Array.isArray(t.modules.lucidBlocksFarmingAndGrowth.growthMilestones) && t.modules.lucidBlocksFarmingAndGrowth.growthMilestones.length > 0 && (
            <div className="scroll-reveal flex flex-wrap gap-3 justify-center mt-8">
      ...
      <section id="vampire-crawlers-best-builds" className="scroll-mt-24 px-4 py-20 bg-white/[0.02]">
        <div className="container mx-auto max-w-5xl">
          ...
          {Array.isArray(t.modules.lucidBlocksBestEarlyUnlocks.items) && t.modules.lucidBlocksBestEarlyUnlocks.items.length > 0 ? (
            <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 gap-4">
              {t.modules.lucidBlocksBestEarlyUnlocks.items.map((item: any, index: number) => {
                const buildIcons = [Home, Sparkles, TrendingUp, Star]
                const Icon = buildIcons[index % buildIcons.length]

                return (
                  <article key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                    <div className="flex items-start justify-between gap-3 mb-4">
                      <div className="w-10 h-10 rounded-lg bg-[hsl(var(--nav-theme)/0.12)] border border-[hsl(var(--nav-theme)/0.3)] flex items-center justify-center">
                        <Icon className="w-5 h-5 text-[hsl(var(--nav-theme-light))]" />
                      </div>
                      {item.recommended_character && (
                    ...
                    {item.deck_shape && (
                      <p className="text-sm text-muted-foreground mb-4">
                        <span className="text-[hsl(var(--nav-theme-light))]">Deck Shape:</span> {item.deck_shape}
                      </p>
                    )}

                    {Array.isArray(item.core_cards) && item.core_cards.length > 0 && (
                      <div className="mb-4">
                        <p className="text-xs uppercase tracking-wide text-[hsl(var(--nav-theme-light))] mb-2">Core Cards</p>
                        <div className="flex flex-wrap gap-2">
                          {item.core_cards.map((cardName: string, cardIndex: number) => (
                            <span key={cardIndex} className="inline-flex items-center px-2 py-1 rounded-md border border-[hsl(var(--nav-theme)/0.3)] bg-[hsl(var(--nav-theme)/0.08)] text-xs">
                              {cardName}
                            </span>
                          ))}
                        </div>
                    ...
                    {Array.isArray(item.priority_evolutions) && item.priority_evolutions.length > 0 && (
                      <div className="mb-4">
                        <p className="text-xs uppercase tracking-wide text-[hsl(var(--nav-theme-light))] mb-2">Priority Evolutions</p>
                        <div className="flex flex-wrap gap-2">
                          {item.priority_evolutions.map((evo: string, evoIndex: number) => (
                            <span key={evoIndex} className="inline-flex items-center gap-1 px-2 py-1 rounded-md border border-[hsl(var(--nav-theme)/0.3)] bg-[hsl(var(--nav-theme)/0.08)] text-xs">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {item.how_to_play || item.description}
                    </p>
                  </article>
                )
              })}
      ...
      <section id="vampire-crawlers-deck-building" className="scroll-mt-24 px-4 py-20">
        <div className="container mx-auto max-w-5xl">
          ...
          {Array.isArray(t.modules.lucidBlocksAchievementTracker.items) && t.modules.lucidBlocksAchievementTracker.items.length > 0 ? (
            <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 gap-4">
              {t.modules.lucidBlocksAchievementTracker.items.map((item: any, index: number) => (
                <div key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-[hsl(var(--nav-theme)/0.2)] border-2 border-[hsl(var(--nav-theme)/0.5)] flex items-center justify-center text-[hsl(var(--nav-theme-light))] font-bold">
                      {item.step || index + 1}
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{item.details || item.description}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {Array.isArray(t.modules.lucidBlocksSingleplayerAndPlatformFAQ.items) && t.modules.lucidBlocksSingleplayerAndPlatformFAQ.items.length > 0 ? (
            <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {t.modules.lucidBlocksSingleplayerAndPlatformFAQ.items.map((item: any, index: number) => {
                const cardIcons = [Star, Sparkles, Gamepad2, Package]
                const Icon = cardIcons[index % cardIcons.length]
                const tierClass =
                  item.tier === 'S'
                    ? 'bg-[hsl(var(--nav-theme-light)/0.2)] border-[hsl(var(--nav-theme-light)/0.45)] text-[hsl(var(--nav-theme-light))]'
                    : item.tier === 'A'
                      ? 'bg-[hsl(var(--nav-theme)/0.18)] border-[hsl(var(--nav-theme)/0.45)] text-[hsl(var(--nav-theme-light))]'
                      : 'bg-[hsl(var(--nav-theme)/0.1)] border-[hsl(var(--nav-theme)/0.3)] text-muted-foreground'
                  ...
                  <article key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                    ...
                    {item.difficulty && (
                      <p className="text-xs text-muted-foreground mb-1">
                        <span className="text-[hsl(var(--nav-theme-light))]">Difficulty:</span> {item.difficulty}
                      </p>
                    )}
                    {item.starter && (
                      <p className="text-xs text-muted-foreground mb-1">
                        <span className="text-[hsl(var(--nav-theme-light))]">Starter:</span> {item.starter}
                      </p>
                    )}
                    {item.unlock && (
                      <p className="text-xs text-muted-foreground mb-3">
                        <span className="text-[hsl(var(--nav-theme-light))]">Unlock:</span> {item.unlock}
                      </p>
                    )}
                    {item.best_for && (
                      <p className="text-sm text-muted-foreground mb-3">
                    ...
                    {Array.isArray(item.signature_pairings) && item.signature_pairings.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {item.signature_pairings.map((pairing: string, pairingIndex: number) => (
                          <span key={pairingIndex} className="inline-flex items-center gap-1 px-2 py-1 rounded-md border border-[hsl(var(--nav-theme)/0.3)] bg-[hsl(var(--nav-theme)/0.08)] text-xs">
                            <Check className="w-3 h-3 text-[hsl(var(--nav-theme-light))]" />
                            {pairing}
                          </span>
                        ))}
      ...
      {/* Module 13: Characters and Unlocks */}
      <section id="vampire-crawlers-characters-and-unlocks" className="scroll-mt-24 px-4 py-20">
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 gap-6">
            {t.modules.lucidBlocksSteamDeckAndController.faqs.map((character: any, index: number) => {
              const characterIcons = [Star, Sparkles, Gamepad2, Package]
              const CharacterIcon = characterIcons[index % characterIcons.length]
              const characterName = character.name ?? character.question ?? `Vampire Crawlers Character ${index + 1}`
              const bestEvolutions = Array.isArray(character.best_evolutions) ? character.best_evolutions : []
              const difficulty = character.difficulty ?? 'Vampire Crawlers Role'
              const unlock = character.unlock ?? 'Track this Vampire Crawlers unlock in progression routes.'
              const starterCard = character.starter_card ?? 'Starter not listed'
              const passive = character.passive ?? character.answer ?? 'Passive details are listed in this Vampire Crawlers roster module.'
              const playstyle = character.playstyle ?? 'Use this Vampire Crawlers profile to match unlock order with your preferred run style.'
              ...
              return (
                <article key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-[hsl(var(--nav-theme)/0.14)] border border-[hsl(var(--nav-theme)/0.35)] flex items-center justify-center">
                        <CharacterIcon className="w-5 h-5 text-[hsl(var(--nav-theme-light))]" />
                      </div>
                      <h3 className="text-xl font-bold">
                        <LinkedTitle linkData={moduleLinkMap[`lucidBlocksSteamDeckAndController::faqs::${index}`]} locale={locale}>
                          {characterName}
                        </LinkedTitle>
                      </h3>
                    </div>
                    <span className="text-xs px-2 py-1 rounded-full bg-[hsl(var(--nav-theme)/0.1)] border border-[hsl(var(--nav-theme)/0.35)] text-[hsl(var(--nav-theme-light))]">
                      {difficulty}
                    </span>
                  </div>

                  <div className="space-y-2 text-sm mb-4">
                    <p><span className="font-semibold text-[hsl(var(--nav-theme-light))]">Unlock:</span> {unlock}</p>
                    <p><span className="font-semibold text-[hsl(var(--nav-theme-light))]">Starter Card:</span> {starterCard}</p>
                    <p className="text-muted-foreground"><span className="font-semibold text-foreground">Passive:</span> {passive}</p>
                  </div>

                  {bestEvolutions.length > 0 && (
                    <div className="mb-4">
                      <p className="text-sm font-semibold text-[hsl(var(--nav-theme-light))] mb-2">Best Evolutions</p>
                      <div className="flex flex-wrap gap-2">
                        {bestEvolutions.map((evolution: string, evolutionIndex: number) => (
                          <span key={evolutionIndex} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-[hsl(var(--nav-theme)/0.3)] bg-[hsl(var(--nav-theme)/0.08)] text-xs">
                            <Check className="w-3 h-3 text-[hsl(var(--nav-theme-light))]" />
                            {evolution}
                          </span>
                        ))}
                  ...
                  <p className="text-sm text-muted-foreground">
                    <span className="font-semibold text-foreground">Playstyle:</span> {playstyle}
                  </p>
                </article>
              )
            })}
      ...
      <section id="vampire-crawlers-card-list" className="scroll-mt-24 px-4 py-20 bg-white/[0.02]">
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12 scroll-reveal">
            <div className="flex items-center justify-center gap-3 mb-4">
              <BookOpen className="w-7 h-7 text-[hsl(var(--nav-theme-light))]" />
              <h2 className="text-4xl md:text-5xl font-bold"><LinkedTitle linkData={moduleLinkMap['lucidBlocksSettingsAndAccessibility']} locale={locale}>{t.modules.lucidBlocksSettingsAndAccessibility.title}</LinkedTitle></h2>
            </div>
            <p className="text-muted-foreground text-lg max-w-3xl mx-auto">{t.modules.lucidBlocksSettingsAndAccessibility.intro}</p>
          </div>

          <div className="hidden md:block scroll-reveal overflow-x-auto border border-border rounded-xl bg-white/5">
            <table className="w-full text-sm">
              <thead className="bg-[hsl(var(--nav-theme)/0.08)]">
                <tr className="text-left">
                  ...
                  <th className="px-4 py-3 font-semibold">Upgrade or Recipe</th>
                  <th className="px-4 py-3 font-semibold">Effect</th>
                </tr>
              </thead>
              <tbody>
                {t.modules.lucidBlocksSettingsAndAccessibility.settings.map((card: any, index: number) => (
                  (() => {
                    const cardName = card.name ?? `Vampire Crawlers Card ${index + 1}`
                    const category = card.category ?? card.type ?? 'General'
                    const cost = card.cost ?? '-'
                    const upgradeOrRecipe = card.upgrade_or_recipe ?? '-'
                    const effect = card.effect ?? card.description ?? '-'

                    return (
                      <tr key={index} className="border-t border-border/70 hover:bg-white/5 transition-colors align-top">
                        ...
                        <td className="px-4 py-3 text-[hsl(var(--nav-theme-light))] font-semibold">{cost}</td>
                        <td className="px-4 py-3 text-muted-foreground">{upgradeOrRecipe}</td>
              ...
              (() => {
                ...
                return (
                  <article key={index} className="p-5 bg-white/5 border border-border rounded-xl">
                    ...
                    <p className="text-xs uppercase tracking-wide text-[hsl(var(--nav-theme-light))] mb-2">{category}</p>
                    <p className="text-sm text-muted-foreground mb-2"><span className="font-semibold text-foreground">Upgrade:</span> {upgradeOrRecipe}</p>
      ...
      {/* Module 15: Dungeon Events */}
      <section id="vampire-crawlers-dungeon-events" className="scroll-mt-24 px-4 py-20">
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12 scroll-reveal">
            <div className="flex items-center justify-center gap-3 mb-4">
              <Clock className="w-7 h-7 text-[hsl(var(--nav-theme-light))]" />
              <h2 className="text-4xl md:text-5xl font-bold"><LinkedTitle linkData={moduleLinkMap['lucidBlocksUpdatesAndPatchNotes']} locale={locale}>{t.modules.lucidBlocksUpdatesAndPatchNotes.title}</LinkedTitle></h2>
            </div>
          ...
          <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 gap-6">
            {t.modules.lucidBlocksUpdatesAndPatchNotes.entries.map((entry: any, index: number) => {
              const eventIcons = [Sparkles, AlertTriangle, Star, Hammer, Package]
              const EventIcon = eventIcons[index % eventIcons.length]
              const eventName = entry.name ?? entry.title ?? `Vampire Crawlers Event ${index + 1}`
              const location = entry.location ?? 'Dungeon side room'
              const reward = entry.reward ?? 'Varies'
              const eventCost = entry.cost ?? 'Varies'
              const eventDescription = entry.description ?? 'Event details are listed in this Vampire Crawlers event module.'
              const bestTime = entry.best_time_to_take ?? 'Take this detour when it supports your current run plan.'
              ...
              return (
                <article key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-[hsl(var(--nav-theme)/0.14)] border border-[hsl(var(--nav-theme)/0.35)] flex items-center justify-center">
                        <EventIcon className="w-5 h-5 text-[hsl(var(--nav-theme-light))]" />
                      </div>
                      <h3 className="font-bold">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm mb-3">
                    <p><span className="font-semibold text-[hsl(var(--nav-theme-light))]">Location:</span> {location}</p>
                    <p><span className="font-semibold text-[hsl(var(--nav-theme-light))]">Reward:</span> {reward}</p>
                    <p><span className="font-semibold text-[hsl(var(--nav-theme-light))]">Cost:</span> {eventCost}</p>
                  </div>

                  <p className="text-sm text-muted-foreground mb-3">{eventDescription}</p>
                  <p className="text-sm text-muted-foreground">
      ...
      {/* Module 16: Steam Deck and Controller Support */}
      <section id="vampire-crawlers-steam-deck-controller-support" className="scroll-mt-24 px-4 py-20 bg-white/[0.02]">
        <div className="container mx-auto max-w-5xl">
          <div className="text-center mb-12 scroll-reveal">
            <div className="flex items-center justify-center gap-3 mb-4">
              <Settings className="w-7 h-7 text-[hsl(var(--nav-theme-light))]" />
              <h2 className="text-4xl md:text-5xl font-bold"><LinkedTitle linkData={moduleLinkMap['lucidBlocksCrashFixAndTroubleshooting']} locale={locale}>{t.modules.lucidBlocksCrashFixAndTroubleshooting.title}</LinkedTitle></h2>
            </div>
          ...
          <div className="hidden md:block scroll-reveal overflow-x-auto border border-border rounded-xl bg-white/5">
            <table className="w-full text-sm">
              ...
              <tbody>
                {t.modules.lucidBlocksCrashFixAndTroubleshooting.steps.map((item: any, index: number) => (
                  (() => {
                    const feature = item.feature ?? item.title ?? `Vampire Crawlers Support Item ${index + 1}`
                    const status = item.status ?? 'Available'
                    const details = item.details ?? item.description ?? 'See this Vampire Crawlers support row for platform details.'
                    ...
                    return (
                      <tr key={index} className="border-t border-border/70 hover:bg-white/5 transition-colors align-top">
                        ...
                        <td className="px-4 py-3 text-[hsl(var(--nav-theme-light))] font-semibold">{status}</td>
